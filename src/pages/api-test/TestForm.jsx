...
class TestForm extends Component {
  constructor(props) {
    super(props);
    this.createHeaderParamFormItems = this.createHeaderParamFormItems.bind(
      this
    );
    ...
    if (clickedApi.bodyParams.length > 0) {
      ...
      for (const singleParam of clickedApi.bodyParams) {
        ...
        if (
          initialValue === "" &&
          "undefined" !== typeof singleParam.type &&
          null !== singleParam.type &&
          singleParam.type.startsWith("array")
        ) {
          initialValue = [];
        }
  ...
  buildObjectInitVal(obj) {
    ...
    for (let i = 0; i < obj.children.length; i++) {
      ...
      if (singleParam.type === "object") {
        ...
        typeof singleParam.children !== "undefined" &&
        null !== singleParam.children &&
        singleParam.children.length > 0
      ) {
  ...
  onSubmitForm(e) {
    ...
    let isUpload = false;
    if (
      typeof clickedApi.fileParams !== "undefined" &&
      clickedApi.fileParams.length > 0
    ) {
    ...
    this.props.form.validateFields((err, values) => {
      let testApiUrl = basePath;
      let tmpWebApiPath = Util.formReplacePathVar(
        clickedApi.path,
        values["__path"]
      );
      ...
      const printResponse = result => {
        ...
        if (result.headers["content-type"]) {
          if (
            -1 !== result.headers["content-type"].indexOf("octet-stream") ||
        ...
        if (isBinaryResult) {
          ...
          link.href = url;
          let fileName =
            "后台返回的下载文件(未在content-disposition中找到文件名)";
          let orignalFileName = null;
          console.log(result.headers);
          if (result.headers["content-disposition"]) {
            let tmp = result.headers["content-disposition"].replace(
              new RegExp("attachment;filename=", "gm"),
              ""
            );
            if ("" !== tmp) {
      ...
      if (clickedApi.produces) {
        for (const singleProducer of clickedApi.produces) {
          if (
            -1 !== singleProducer.indexOf("octet-stream") ||
            ...
            -1 !== singleProducer.indexOf("image/jpeg") ||
      ...
      if (clickedApi.method.toUpperCase() === "POST") {
        if (isUpload) {
          ...
          let normalFormData =
            typeof values["__form"] !== "undefined" ? values["__form"] : {};
          let formData = objectToForm(normalFormData, previousFormData);
          apiRemoteService
            .postUpload(testApiUrl, formData, values["__header"])
          ...
          if (isBinaryResult) {
            ...
            apiRemoteService
              .normalBodyPost(testApiUrl, values["__bodyForm"], headers)
              .then(function(result) {
                printResponse(result);
              });
          }
          ...
          // post使用普通的表单传参
          apiRemoteService
            .normalFormPost(testApiUrl, values["__form"], headers)
            .then(function(result) {
              printResponse(result);
            });
        }
        ...
        if (isImageResult) {
          ...
          apiRemoteService
            .normalGet(testApiUrl, values["__form"], headers)
            .then(function(result) {
              printResponse(result);
            });
        }
        ...
        if (Util.strNotBlank(values["__bodyForm"])) {
          apiRemoteService
            .normalBodyPut(testApiUrl, values["__bodyForm"], headers)
            .then(function(result) {
              printResponse(result);
            });
        } else {
          apiRemoteService
            .normalFormPut(testApiUrl, values["__form"], headers)
            .then(function(result) {
              printResponse(result);
            });
        }
        ...
        apiRemoteService
          .normalDelete(testApiUrl, values["__form"], headers)
          .then(function(result) {
            printResponse(result);
          });
      }
