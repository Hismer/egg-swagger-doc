...
class ApiListUI extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (
      nextProps.placeholder !== this.state.placeholder ||
  ...
  buldMenu() {
    ...
    if (queryApiInfoMap) {
      for (const apiName in queryApiInfoMap) {
        menuArr.push(
          <SubMenu
            key={queryApiInfoMap[apiName].key}
            title={<span>{apiName}</span>}
          >
  ...
  buildMenuItem(apiArr) {
    ...
    for (const apiInfo of apiArr) {
      if (apiInfo.deprecated) {
        menuArr.push(
          <Menu.Item key={apiInfo.key} title={apiInfo.name}>
  ...
  onApiQueryHandler(e) {
    ...
    if ("" === val) {
      ...
      const { openedKeys, queriedApiInfoMap } = menuSearch(
        apiInfoMap,
        val.toUpperCase()
      );
      this.setState({
  ...
  menuItemClickHandle({ item, key }) {
    ...
    const newPathName = `/blank/${this.props.type}/${encodeURIComponent(
      this.props.url
    )}/${tagId}/${key}`;
  ...
  render() {
    ...
    const animateClassName = this.state.queryApiInfoMap
      ? "animated fadeInDown"
      : "";
    return (
