...
import styles from "./ToolbarUI.css";

// UI组件
class ToolbarUI extends Component {
  constructor(props) {
    super(props);
    const lanMenu = props.langArr.map((item, index) => (
      <Menu.Item key={index}>
        <a onClick={e => props.onLanChagne(item.flag)}>{item.txt}</a>
      </Menu.Item>
    ));
    this.state = {
      langMenu: <Menu>{lanMenu}</Menu>
    };
  }
      ...
      <ul className={styles.toolbarPanel}>
        <li>
          <Dropdown overlay={langMenu}>
            <a className="ant-dropdown-link" href="#">
              <span />
              <Icon type="flag" theme="outlined" style={{ fontSize: 20 }} />
            </a>
          </Dropdown>
        </li>
        {/*<li>*/}
        ...
        <li>
          <a
            ...
            href="#"
            onClick={this.props.onTestHandle}
          >
            <span />
            <Icon type="api" theme="outlined" style={{ fontSize: 20 }} />
          </a>
        </li>
      </ul>
    );
