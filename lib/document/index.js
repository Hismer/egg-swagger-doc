...
function getTag_Path(fileDir, securitys, swagger, definitions, baseDir) {
  // 已存在tag集合
  let tagNames = [];
  let tags = [];
  let paths = {};
  ...
  for (let name of names) {
    ...
    if (stat.isFile() && ['.js', '.ts'].indexOf(path.extname(name)) !== -1) {
      ...
      for (let i = 1; i < blocks.length; i++) {
        ...
        let direct = filepath
          .split(/\.(js|ts)/)[0]
          .split(`${baseDir}${path.sep}app${path.sep}`)[1];
        ...
        if (routers) {
          ...
          const reg = path.sep === '/' ? new RegExp(path.sep, 'g') : path.sep;
          path_method.operationId = `${direct.replace(reg, '-')}-${
            func[i - 1]
          }`;
          path_method.consumes = generateConsumes(blocks[i], swagger);
          path_method.produces = generateProduces(blocks[i], swagger);
          path_method.parameters = generateRequest(
            blocks[i],
            routers,
            definitions
          );
          path_method.security = generateSecurity(
            blocks[i],
            securitys,
            swagger
          );
          path_method.responses = generateResponse(
            blocks[i],
            routers,
            definitions
          );
...
function generateResponse(block, routers, definitions) {
  ...
  if (responses) {
    for (let response of responses) {
      ...
      if (response[2] && response[2] !== 'undefined') {
...
function generateParameters(request, routers, definitions) {
  ...
  if (
    parameter.in.toLowerCase() === 'body' &&
    !_.itemType.includes(request[2])
  ) {
    let schema = {};
