...
function contractLoader(app, baseDir, directory) {
  ...
  for (let name of names) {
    ...
    if (
      stat.isFile() &&
      ['.js', '.ts'].indexOf(path.extname(filepath)) !== -1
    ) {
      let def = require(filepath.split(/\.(js|ts)/)[0]);
      for (let object in def) {
        const data = {};
        for (const key in def[object]) {
          data[key] = Object.assign({}, def[object][key]);
          delete data[key].render;
        }
        CONTRACT[object] = {
  ...
  for (let object in source) {
    ...
    for (let field in def) {
      ...
      if (!def[field].hasOwnProperty('type')) {
        throw new Error(
          `[egg-doc] ${path}: ${object}.${field}.type is necessary`
        );
      }
      ...
      if (def[field].type === 'array') {
        if (!def[field].hasOwnProperty('itemType')) {
          throw new Error(
            `[egg-doc] ${path}: ${object}.${field}.itemType is necessary`
          );
        }
...
function buildValidateRule(source) {
  ...
  for (let object in source) {
    ...
    for (let field in def) {
      ...
      if (
        def[field].hasOwnProperty('format') &&
        def[field].format === 'date-time' &&
        def[field].format === 'date'
      ) {
        delete def[field].format;
      }
