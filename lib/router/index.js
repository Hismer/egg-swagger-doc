...
module.exports = {
  ...
  basicRouterRegister: app => {
    ...
    app.get(`${basePath}/swagger-doc`, ctx => {
      ctx.response.status = 200;
      ctx.response.type = 'application/json';
      ctx.response.body = documentInit(app);
    });
    app.logger.info(
      `[egg-doc] register router: get http://127.0.0.1${basePath}/swagger-doc`
    );
    // swaggerUI的静态资源加入缓存，配置访问路由
    const swaggerH5 = path.join(__dirname, '../../app/public');
    app.use(staticCache(swaggerH5, { prefix: basePath }, {}));
    app.logger.info(
      `[egg-doc] register router: get http://127.0.0.1${basePath}/swagger-ui.html`
    );
  },
