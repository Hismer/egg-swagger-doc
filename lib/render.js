...
const baseValueRenders = {
  integer(entity) {
    ...
    return value ? parseInt(entity[this.key]) : undefined;
  },
  string(entity) {
    const value = entity[this.key];
    return value ? entity[this.key].toString() : undefined;
  },
  boolean(entity) {
    const value = entity[this.key];
    return value ? Boolean(entity[this.key]) : undefined;
  },
  number(entity) {
    const value = entity[this.key];
    return value ? Number(entity[this.key]) : undefined;
  },
};
